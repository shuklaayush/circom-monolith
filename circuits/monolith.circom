...
template concrete(round) {
    ...
    for (var row = 0; row < SPONGE_WIDTH(); row++) {
        ...
        for (var column = 0; column < SPONGE_WIDTH(); column++) {
            tmp += stateIn[column] * MAT_12(row, column); // TODO: goldilocks mul
        }
        stateOut[row] <== tmp + ROUND_CONSTANTS(round, row); // TODO: goldilocks add
    }
}
...
template bricks() {
    ...
    for (var i = 1; i < SPONGE_WIDTH(); i++) {
        stateOut[i] <== stateIn[i - 1] * stateIn[i - 1]; // TODO: goldilocks mul
    }
}
